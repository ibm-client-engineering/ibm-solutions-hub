...
const RecentProjectsTiles = ({ data }) => {
  return (
    <>
      {data.map((repo, index) => (
        <Column lg={4} md={2} sm={1}><ClickableTile id={repo.name} className="projectTile" key={repo.name} href={repo.homepageUrl} target="_blank" rel="noopener noreferrer" renderIcon={Launch} style={{ height: '400px' }}>
          ...
          {repo.repositoryTopics.nodes.map((nodes, index) => (
                    <Tag className="projectTile__topics" key={nodes.topic.name}>
                        {nodes.topic.name}
                    </Tag>
                ))}
